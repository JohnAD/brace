...
hashtable struct__sdl_keys_by_name
hashtable *sdl_keys_by_name = &struct__sdl_keys_by_name
sdl_key *sdl_keys_by_code[SDLK_LAST]

sdl_keys_init()
	hashtable_init(sdl_keys_by_name)
	sdl_key *p
	sdl_key *sdl_keys_table_end = array_end(sdl_keys_table)
	for p = sdl_keys_table; p!=sdl_keys_table_end; ++p
		put(sdl_keys_by_name, p->name, p)
		sdl_keys_by_code[p->code] = p

SDLKey sdl_key_code_by_name(cstr name)
	sdl_key *k = get(sdl_keys_by_name, name)
	return k ? k->code : SDLK_UNKNOWN

cstr sdl_key_name_by_code(SDLKey code)
	if code < SDLK_LAST
		sdl_key *k = sdl_keys_by_code[code]
		if k
			return k->name
	return NULL
